...
export default function NewlyScrapedEvents() {
  ...
  const handleApprove = async (id) => {
    try {
      await updateDoc(doc(db, 'events', id), { status: 'active' });
          ...
          <table className="min-w-full divide-y divide-slate-200">
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {loading ? (
                ...
                <tr><td colSpan="4" className="px-6 py-4 text-center text-slate-500">No newly scraped events pending approval.</td></tr>
              ) : (
                events.map((event) => (
                  <tr key={event.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button onClick={() => handleApprove(event.id)} className="text-green-600 hover:text-green-900 bg-green-50 px-3 py-1 rounded-md border border-green-200 mr-2">
